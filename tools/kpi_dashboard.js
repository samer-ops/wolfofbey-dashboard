...
const monthStart = fromIdx >= 0 && args[fromIdx+1] ? args[fromIdx+1] : localDateStr(new Date(now.getFullYear(), now.getMonth(), 1));
const monthEnd = toIdx >= 0 && args[toIdx+1] ? args[toIdx+1] : localDateStr(now);
...
function runFilter(preset){
  ...
  let from,to=ld(now);
  if(preset==='this-month') from=ld(new Date(y,m,1));
  else if(preset==='last-month'){from=ld(new Date(y,m-1,1));to=ld(new Date(y,m,0));}
  else if(preset==='last-3') from=ld(new Date(y,m-2,1));
  ...
  if(preset!=='custom'&&event&&event.target)event.target.classList.add('active');
  const cmd='node tools/kpi_dashboard.js --from '+from+' --to '+to;
  const el=document.getElementById('filter-cmd');
  el.style.display='block';
  el.innerHTML='<span style="color:#8899AA">Command copied to clipboard:</span> '+cmd+'<br><span style="color:#27AE60">Paste in terminal and press Enter to refresh dashboard with new dates.</span>';
